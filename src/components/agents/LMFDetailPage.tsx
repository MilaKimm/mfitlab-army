...
const features = [
  { icon: <MessageSquare size={20} />, title: "Slack에서 대화로 실험", desc: "\"리서치 해줘\" 한마디로 12단계 프로세스가 시작됩니다. Slack에서 바로 사용 (Teams 연동 로드맵)." },
  { icon: <ImageIcon size={20} />, title: "AI 이미지/카피 자동 생성", desc: "브랜드 가이드라인을 반영한 광고 이미지, 카피, 영상을 자동 생성하고 평가합니다." },
  { icon: <Target size={20} />, title: "Meta 캠페인 자동화", desc: "타겟/예산/기간 설정 후 캠페인 자동 생성 및 라이브. \"알아서 해줘\" 모드로 즉시 집행 가능." },
  { icon: <BarChart3 size={20} />, title: "성공 방정식 도출", desc: "반복 가능한 성공 공식과 지양할 실패 공식을 데이터로 정리. 다음 실험 액션까지 자동 제안." },
  { icon: <Sparkles size={20} />, title: "다국어 소재 생성", desc: "한국어·일본어 광고 소재를 한 번에 생성 (영어·중국어 확장 예정). 글로벌 캠페인도 1명이 운영." },
];
...
export default function LMFDetailPage({ agent }: Props) {
    ...
    <div className="pt-20">
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
            <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: agent.color }}>Language-Market Fit</span>
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mt-2">{agent.name}</h1>
            <p className="text-lg text-gray-600 mt-4 max-w-2xl mx-auto">마케터 1명이 광고 전 과정을 자동화합니다</p>
            <p className="text-sm text-gray-500 mt-3 max-w-xl mx-auto">LMF(Language-Market Fit) — 시장이 반응하는 언어를 데이터로 찾는 프레임워크</p>
          </motion.div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-5xl mx-auto px-6">
          <FadeInOnScroll>
            <h2 className="text-2xl font-bold text-gray-900 text-center mb-2">AI 에이전트와 대화하면 캠페인이 완성됩니다</h2>
      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-6">
          <FadeInOnScroll>
            <h2 className="text-2xl font-bold text-gray-900 text-center mb-2">AI가 만든 광고 소재 예시</h2>
            <p className="text-sm text-gray-500 text-center mb-10">브랜드 가이드라인과 퍼소나를 반영한 소재를 자동 생성합니다</p>
          </FadeInOnScroll>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {creativeExamples.map((ex, i) => (
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-5xl mx-auto px-6">
          <FadeInOnScroll>
            ...
            <p className="text-sm text-gray-500 text-center mb-10">같은 인원으로 광고 성과가 달라집니다</p>
          </FadeInOnScroll>
            ...
            <FadeInOnScroll>
              <div className="rounded-xl bg-white border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-bold text-gray-900">광고 라이브까지 소요 시간</p>
                  <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ backgroundColor: `${agent.color}15`, color: agent.color }}>5x 단축</span>
                </div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <div className="h-8 bg-gray-200 rounded-lg flex items-center px-3" style={{ width: "100%" }}>
                      <span className="text-[11px] font-bold text-gray-600 whitespace-nowrap">10시간+</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <motion.div initial={{ width: 0 }} whileInView={{ width: "20%" }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" }} className="h-8 rounded-lg flex items-center px-3" style={{ backgroundColor: agent.color }}>
                      <span className="text-[11px] font-bold text-white whitespace-nowrap">2시간</span>
                    </motion.div>
                  </div>
                </div>
              </div>
      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-6">
          <FadeInOnScroll>
            ...
            <p className="text-sm text-gray-500 text-center mb-10">감으로 만들던 소재를, 데이터로 설계합니다</p>
          </FadeInOnScroll>
